...
export default async function HistoricoPage({ searchParams }: Props) {
  ...
  const [{ data: checklists = [] }, { data: ncs = [] }] = await Promise.all([
    showChecklist
      ? supabase.from('checklists').select('id, veiculo, status, created_at').order('created_at', { ascending: false })
      : Promise.resolve({ data: [] }),
    showNC
      ? supabase.from('nao_conformidades').select('id, descricao, status, created_at').order('created_at', { ascending: false })
      : Promise.resolve({ data: [] }),
  ])

  const items = [
    ...(checklists ?? []).map((c) => ({ id: c.id, title: c.veiculo, type: 'Checklist', status: c.status, created_at: c.created_at })),
    ...(ncs ?? []).map((n) => ({ id: n.id, title: n.descricao, type: 'Não conformidade', status: n.status, created_at: n.created_at })),
